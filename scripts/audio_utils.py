...
def trim_relative_silence_from_audio(audio, sr, frame_duration=0.04, hop_duration=0.01):
    ...
    rms = librosa.feature.rms(
        S=S, frame_length=frame_length, hop_length=hop_length, pad_mode="constant"
    ).flatten()
    ...
    end_idx = min(
        int(ceil(min(active_idxs[-1] + 1, rms.shape[0]) * hop_duration * sr)),
        audio.shape[0],
    )
