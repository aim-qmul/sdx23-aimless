...
from data import CodedMUSDB
from data import FastMUSDB
from data import DnR as DnRDataset
from augment.cpu import *


class MUSDBEncodec(pl.LightningDataModule):
    def __init__(self, root: str, seq_len: int, batch_size: int) -> None:
        super().__init__()
        self.save_hyperparameters()

    def setup(self, stage=None):
        self.train_dataset = CodedMUSDB(
            os.path.join(self.hparams.root, "train"),
            seq_len=self.hparams.seq_len,
        )
        self.val_dataset = CodedMUSDB(
            os.path.join(self.hparams.root, "test"),
            seq_len=self.hparams.seq_len,
        )
